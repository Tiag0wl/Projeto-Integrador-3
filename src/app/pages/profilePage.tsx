...
interface ProfilePageProps { 
  ...
  onOpenOccurrence: (occurrenceId: number | string) => void | Promise<void>;
} 
          ...
          {(myReports.length > 0 || myUserReports.length > 0) ? ( 
            <div className="space-y-3"> 
              {myReports.map((report) => ( 
                <div 
                  key={`occurrence-${report.id}`} 
                  onClick={() => onOpenOccurrence(report.id)}
              ...
              {myUserReports.map((report) => ( 
                <div 
                  ...
                  onClick={() => onOpenOccurrence(report.occurrence_id)}
                  className="border border-green-100 rounded-md p-4 bg-green-50/30 cursor-pointer hover:bg-green-50 transition-colors"
                > 
                  <div className="flex items-center justify-between gap-3 mb-2"> 
